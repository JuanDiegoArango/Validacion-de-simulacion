...
struct SimulationParameters {
    ...
    T resolution;                               // Total number of lattice nodes in the characteristic length.
    ...
    Array<T,3> Velocidad_Inicial;
    Array<T,3> Velocidad_Angular_Inicial;
...
void readUserDefinedSimulationParameters(std::string xmlInputFileName, SimulationParameters& param)
{
    ...
    document["numerics"]["U_inicial"].read<T,3>(param.Velocidad_Inicial);
    document["numerics"]["W_inicial"].read<T,3>(param.Velocidad_Angular_Inicial);
...
void calculateDerivedSimulationParameters(SimulationParameters& param)
{
    ...
    param.dx = param.characteristicLength / (param.resolution);
    ...
    param.velocidad=param.dt/param.dx*param.Velocidad_Inicial;
    param.velocidad_angular=param.dt*param.Velocidad_Angular_Inicial;
    ...
    param.nu_LB = param.nu * (param.dt)/(param.rho*param.dx * param.dx);
    
    
    //este parametro es el basicamente controla
    param.omega = 1.0 / (DESCRIPTOR<T>::invCs2 * param.nu_LB + 0.5);
    ...
    pcout << "velocidad del sonido = " << param.nu_LB*(1.0/param.omega-0.5) << std::endl;
...
void initializeImmersedSurfaceData(SimulationParameters& param)
{
    ...
    for (plint iSurface = 0; iSurface < param.numSurfaces; iSurface++) {
        ...
        surfaceTriangleSet->scale(12.5/(4*param.dx));
        Array <T,3> posicion_inicial=Array<T,3>((param.nx-1)*0.5,(param.ny-1)*0.5,1.5/(param.dx)+1);
        ...
        param.radio_bacteria= 0.5 *std::abs(el_cubo.lowerLeftCorner[2] - el_cubo.upperRightCorner[2]);
...
int main(int argc, char* argv[])
{
    ...
    T a=param.rho*pow(param.dx,4)/pow(param.dt,2);
    T b=a*param.dx;
    ...
    for (plint iIter = iniIter; iIter < param.maxIter && !stopExecution; iIter++) {
        ...
        if (iIter>iniIter)
        {
            ...
            force =  2.5*-reduceImmersedForce<T>(*container, 0);
        ...
        param.velocidad=force*(param.rho*pow(param.dx,3)/param.masa)+param.velocidad;
        
        //vector velocidad angular después.
        param.velocidad_angular=torque*(param.rho*pow(param.dx,3)/param.momento_de_inercia)+param.velocidad_angular;
        ...
        if (norma!=0.0){param.vectores_unitarios=param.velocidad_angular/norma;}
       ...
       if (iIter>4000)
       {
         ...
         plint numVertices = param.allSurfaces[0].getNumVertices();
         plint startId = param.startIds[0];
         ...
         for (plint iVertex = 0; iVertex < numVertices; iVertex++)
         {
             ...
             if (norma!=0){position = getRotatedPosition(position, param.velocidad_angular,param.vectores_unitarios, param.centro_geometrico);}
        ...
        if (iIter>iniIter)
        {
            ...
            pcout<< iIter<<","<<param.velocidad[0]<<","<<param.velocidad[1]<<","<<param.velocidad[2]<<","<<param.velocidad_angular[0]<<","<<param.velocidad_angular[1]<<","<<param.velocidad_angular[2] <<","<<a*force[0]<<","<<b*torque[1]<<std::endl;
        
            pcout<< iIter<<","<<a*force[0]<<","<<b*torque[1]<<std::endl;
